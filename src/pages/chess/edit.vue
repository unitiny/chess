...
async function confirm() {
  ...
  if (flag) {
    sendMachineAction(MACHINE_ACTION.CLEAR)
...
function sendStep(step: Objects) {
  ...
  // 游戏模式决定发送模式
  if (!isDoubleGame(mode.value) && !common.hasStatus(mode.value, "10")) {
    // 人机红方则翻转走法
    if (common.isSameStatus(mode.value, "00")) {
      start = [8 - start[0], 9 - start[1]]
      end = [8 - end[0], 9 - end[1]]
    }
    ...
    WS.sendMachineMsg(msg)
    return
  } else if (!isDoubleGame(mode.value) && common.isSameStatus(mode.value, "11", 1)) {
    return;  // 若为自定义玩家对战则不发送信息
  } else {
...
// 发送给电脑指令
function sendMachineAction(action: number) {
  let msg: Objects = {
    "action": action,
    "room": store.state.room,
  }
...
function connect(): Chat {
  ...
  let url = `${API.chat}?name=${name}&room=${room}&action=${action}`
  return new Chat(url)
}
    ...
    (val) => {
      ...
      switch (WS.message.action) {
        ...
        case 1:
          ...
          } else if (WS.message.name === MACHINE_NAME) {
            if (common.isJSON(WS.message.message)) {
              console.log("machine chess move")
              emit("machineStep", WS.message.message)
            }
          }
          break
        case 2:
...
defineExpose({sendStep, isDoubleGame, sendAction, sendMachineAction})
