...
function SoldierMoves(chesses: Array<Chess>, index: number): Array<Move> {
    ...
    for (let i = 0; i < legalPos.length; i++) {
        ...
        try {
            // 无报错则表示合法走法
            rules.Soldier(start, target, soldier.bottom)
            let move: Move = {
...
function HorseMoves(chesses: Array<Chess>, index: number): Array<Move> {
    ...
    for (let i = 0; i < legalPos.length; i++) {
        ...
        try {
            // 无报错则表示合法走法
            rules.Horse(chesses, start, target, horse.bottom)
            let move: Move = {
...
function ElephantMoves(chesses: Array<Chess>, index: number): Array<Move> {
    ...
    for (let i = 0; i < legalPos.length; i++) {
        ...
        try {
            // 无报错则表示合法走法
            rules.Elephant(chesses, start, target, elephant.bottom)
            let move: Move = {
...
function ScholarMoves(chesses: Array<Chess>, index: number): Array<Move> {
    ...
    for (let i = 0; i < legalPos.length; i++) {
        ...
        try {
            // 无报错则表示合法走法
            rules.Scholar(start, target, scholar.bottom)
            let move: Move = {
...
function KingMoves(chesses: Array<Chess>, index: number): Array<Move> {
    ...
    for (let i = 0; i < legalPos.length; i++) {
        ...
        try {
            // 无报错则表示合法走法
            rules.King(chesses, start, target, king.bottom)
            let move: Move = {
...
function move(chesses: Array<Chess>, step: Step): number {
    // 获取被吃棋子的索引
    let die = getChessByPos(chesses, step.Move.end)
